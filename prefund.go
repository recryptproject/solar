...
func init() {
	cli := app.Command("prefund", "(recrypt) fund an owner address with uxtos")
	...
	appTasks["prefund"] = func() (err error) {
		...
		/*
			...
			2. "amounts"             (string, required) A json object with addresses and amounts
			    {
			      "address":amount   (numeric or string) The recrypt address is the key, the numeric amount (can be string) in RECRYPT is the value
			      ,...
			    }
		*/
